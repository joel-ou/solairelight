package cn.muskmelon.forward;

import cn.muskmelon.expression.ExpressionEvaluator;
import cn.muskmelon.expression.Operator;
import cn.muskmelon.expression.SpringExpressionEvaluator;
import cn.muskmelon.properties.MuskmelonProperties;
import cn.muskmelon.properties.Route;
...
public class ForwardService {
    ...
    public void forward(WebSocketSessionExpand sessionExpand, Object message){
        ...
        try {
            if (!muskmelonProperties.getForward().isEnable()) {
            ...
            if (uri == null) {
                log.warn("no route matched. message: {}  config: {}", jsonObject, muskmelonProperties.getForward());
                return;
            }
            ForwardWebClient
            ...
            log.error("forward error occurred. message: {}  config: {}", jsonObject, muskmelonProperties.getForward(), e);
        }
    }
    ...
    public String routing(WebSocketSessionExpand sessionExpand, WebSocketMessage webSocketMessage,
        ...
        for (Route route : muskmelonProperties.getForward().getRoutes()) {
    ...
    private MultiValueMap<String, String> headerHandle(BasicSession basicSession){
        String forwardHeader = muskmelonProperties.getForward().getForwardHeader();
